"""
Dense Trigonometric Function testcases
Num unknowns : 500
"""
...
P.add_nonlin_neuron('sin', NUM_SIN)
P.add_nonlin_neuron('cos', NUM_COS)
P.parse()
########

## Neural network section
IN_VECT = np.array([[1]])
NET = FFNN.FFNN([1, NUM_UNKNOWNS, NUM_UNKNOWNS+NUM_SIN+NUM_COS, NUM_UNKNOWNS], manh=True, eta=0.1)
...
NET.layers[1].custom_f_act = [afs.identity]*NUM_UNKNOWNS + [afs.sin_sim]*NUM_SIN + [afs.cos_sim]*NUM_COS
NET.layers[1].custom_f_act_d = [afs.identity_d]*NUM_UNKNOWNS + [afs.sin_d]*NUM_SIN + [afs.cos_d]*NUM_COS
...
start = time.time()
for i in range(5):
    train_nn_manh(NET, IN_VECT, OUT_VECT, max_iter=18000, tol_max=2, export_file=False,
                plt=False, num_ukwns=NUM_UNKNOWNS, stop_after=500, debug=False)

    # Invoke the trainer [Adam]
    train_nn_gd(NET, IN_VECT, OUT_VECT, max_iter=5000, export_file=False,
