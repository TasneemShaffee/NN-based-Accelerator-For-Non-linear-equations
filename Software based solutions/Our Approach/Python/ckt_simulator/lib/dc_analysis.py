...
def dc_solve(mna, Ndc, circ, Ntran=None, Gmin=None, x0=None, time=None,
             MAXIT=None, locked_nodes=None, skip_Tt=False, verbose=3, show_eqn=0):
    ...
    while(not converged):
        ...
        try:
            (x, error, converged, n_iter, convergence_by_node) = mdn_solver(x, mna_to_pass, circ, T=N_to_pass,
                                                                            nv=nv, print_steps=(verbose > 0), locked_nodes=locked_nodes, time=time, MAXIT=MAXIT, debug=(verbose == 6), show_eqn=show_eqn)
...
def op_analysis(circ, x0=None, guess=True, outfile=None, verbose=3):
    ...
    **Returns:**
    ...
    (x1, error1, solved1, n_iter1) = dc_solve(mna, N,
                                              circ, Gmin=Gmin_matrix, x0=x0, verbose=verbose, show_eqn=1)
...
def mdn_solver(x, mna, circ, T, MAXIT, nv, locked_nodes, time=None,
    ...
    **Returns:**
    ...
    if sparse:
        ...
        #mna = scipy.sparse.csr_matrix(mna)
        ...
        if nonlinear_circuit:
            ...
            for elem in circ:
                if elem.is_nonlinear:
                    _update_J_and_Tx(J, Tx, x, elem, time, Tx_sym)
            if iteration == 1 and show_eqn==1:
                from . import eqn_generator
                eqn_generator.show_mna(mna, Tx_sym, T, print_res=False)
...
def _update_J_and_Tx(J, Tx, x, elem, time, Tx_sym=None):
    ...
    for index in range(len(out_ports)):
        ...
        if hasattr(elem, 'gstamp') and hasattr(elem, 'istamp'):
            ...
            Tx[iis] += i.reshape(-1)
            if Tx_sym != None:
                Tx_sym[iis[0][0]] += eqn_generator.show_sym(elem) # add the symbolic eqn
                if len(iis[0])>1:
                    Tx_sym[iis[0][1]] += ' - '+eqn_generator.show_sym(elem)[3:] # add the symbolic eqn
            continue
        if n1 or n2:
